...
use crate::messenger::{Data, FileType, Message, MessagesList, Text, Image, common};
...
pub fn parse(path: &str) -> Result<MessagesList, Box<dyn Error>> {
    ...
    let dir = path.parent().expect("The file must be somewhere in a directory");
    
    let mut parents: (String, Vec<OwnedAttribute>) = ("".to_string(), vec![]);
    let mut session = MsgPlusSession::default();
    let mut list = MessagesList {
        recipient_id: path.file_stem().unwrap_or_default().to_str().unwrap_or_default().to_string(),
    ...
    for e in parser {
        match e {
            Ok(XmlEvent::StartElement { name, attributes, .. }) => {
                parse_node(&name.local_name, &parents, &attributes, &mut session, &mut list, dir)?;
...
fn parse_node(name: &str, parents: &(String, Vec<OwnedAttribute>), attributes: &Vec<OwnedAttribute>, session: &mut MsgPlusSession, list: &mut MessagesList, dir: &Path) -> Result<(), Box<dyn Error>> {
    let attributes = parse_attributes(attributes);
    match name {
        "div" => {
            if parents.0.ends_with("html.body")
                && attributes.iter().any(|(attr, val)| attr.eq(&"class") && val.eq(&"mplsession")) {
        ...
        "td" => {
            if parents.0.ends_with("html.body.div.table.tbody.tr") && attributes.iter().any(|(attr, ..)| attr.eq(&"style")) {
                if let Some(style) = attributes.get("style") {
                    session.message_style = html_escape::decode_html_entities(style).trim().to_string()
                }
            }
        }
        "tr" => {
            if parents.0.ends_with("html.body.div.table.tbody") {
                ...
                if attributes.iter().any(|(attr, val)| attr.eq(&"class") && val.eq(&"msgplus")) {
        ...
        "img" => {
            if parents.0.ends_with("html.body.div.table.tbody.tr.td") && attributes.iter().any(|(attr, ..)| attr.eq(&"src")) {
...
fn parse_text(data: &str, parents: &(String, Vec<OwnedAttribute>), list: &mut MessagesList, session: &mut MsgPlusSession) -> Result<(), Box<dyn Error>> {
    match parents.0.as_str() {
        ".html.body.div.ul.li" => {
            let attributes = parse_attributes(&parents.1);
            if attributes.iter().any(|(attr, val)| attr.eq(&"class") && val.eq(&"in")) {
        ...
        ".html.body.div.table.tbody.tr.th.span" => {
            if list.messages.len() == 1 {
                let datetime = NaiveDateTime::new(session.date.date(), NaiveTime::parse_from_str(format!("{}:{}", data, session.date.second()).as_str(), "(%H:%M):%S")?);
                let msg = list.messages.last_mut().unwrap();
                msg.datetime = datetime.format("%Y-%m-%dT%H:%M:%S").to_string();
            } else {
                let datetime = NaiveDateTime::new(session.date.date(), NaiveTime::parse_from_str(data, "(%H:%M)")?);
                let msg = list.messages.last_mut().unwrap();
        ...
        ".html.body.div.table.tbody.tr.td" => {
            ...
            if let Some(Data::System(_)) = msg.data.first() {
                ...
                match attributes.get("style") {
                    None => { txt.style = session.message_style.clone(); }
                    Some(style) => { txt.style = style.trim().to_string(); }
                };
        ...
        let expected = MessagesList {
            ...
            messages: vec![
                ...
                Message {
                    ...
                    data: vec![
                        Data::System("Alice is now offline".to_string())
                    ]
